...
ws <- 
  enframe(list.files(gcsdir,"*.xlsx")) %>% 
  mutate(filename=value,path=gcsdir) %>%
  separate(value,c("drop","geography","language","tag","dateext"),sep="-") %>%
...
The survey asks about point-in-time expectations. A new wave is launched every Friday. The list below provides the dates of collection for each wave. Currently, data are available covering the period between `r dates.final.begin` and `r dates.final.end`.
...
print.part <- function(tagselect,geo,lang) {
  tmp %>% filter(tag==tagselect & geography == geo & language == lang) %>% 
...
```{r details_b_c_f}
print.part("businesses","canada","fr")
```

##### People, Canada, French

```{r details_p_c_f}
print.part("people","canada","fr")
```

##### Business, US, English

```{r details_b_u_e}
print.part("businesses","us","en")
```

##### People, US, English

```{r details_p_u_e}
print.part("people","us","en")
```
